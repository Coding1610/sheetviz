  ...
  return (
    <div className="space-y-6">
      <Card className="p-6">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between mb-6">
          <div>
            <div className="flex items-center">
              <div className="bg-brand-purple-soft p-2 rounded-lg mr-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{Math.round(file.size / 1024)} KB • {previewData.length > 0 ? `${headers.length} columns` : 'No data'}</p>
              </div>
            </div>
          </div>
              ...
              {previewData.map((row, rowIndex) => (
                <tr key={rowIndex} className="hover:bg-gray-50">
                  {headers.map((header, colIndex) => (
                    <td key={`${rowIndex}-${colIndex}`} className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                      {row[header]?.toString() || ''}
                    </td>
                  ))}
                </tr>
        ...
        {previewData.length > 0 && <p className="text-sm text-gray-500 mt-2">Showing first 10 rows</p>}
      </Card>
